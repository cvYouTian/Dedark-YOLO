...
from core.tal import make_anchors, dist2bbox
...
class YOLOv8l(nn.Module):
    def __init__(self, nc, reg_max):
        ...
        #
        self.pred_sbbox = torch.empty(0)
        self.pred_mbbox = torch.empty(0)
        self.pred_lbbox = torch.empty(0)
    ...
    def forward(self, input_data):
        ...
        #
        #         # 输入的是两个输入，一个是加暗图像，另一个是通过CNN之后的特征图
        ...
        self.pred_sbbox = x15 = self.c2f_15(x14)

        x16 = self.conv_16(x15)
        x17 = torch.cat((x12, x16), 1)
        self.pred_mbbox = x18 = self.c2f_18(x17)

        x19 = self.conv_19(x18)
        x20 = torch.cat((sppf, x19), 1)

        self.pred_lbbox = x21 = self.c2f_21(x20)

        # return self.pred_sbbox, self.pred_mbbox, self.pred_lbbox
    ...
    def decoder(self, conv_output, anchors, stride):
        ...
        conv_output = rearrange(conv_output, 'b (anchor classes) h w -> b h w anchor classes',
                                classes=5 + self.num_class, anchor=self.anchor_per_scale)
        ...
        xy_grid = xy_grid.unsqueeze(0).unsqueeze(3).repeat(batch_size, 1, 1, self.anchor_per_scale, 1).float().to(
            "cuda:0")

        pred_xy = (torch.sigmoid(conv_raw_dxdy) + xy_grid) * stride
        pred_wh = (torch.exp(conv_raw_dwdh) * torch.from_numpy(anchors).to("cuda:0").unsqueeze(0).unsqueeze(
            1).unsqueeze(2)) * stride
