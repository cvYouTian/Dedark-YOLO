...
class WarmupCosineAnnealingLR(torch.optim.lr_scheduler._LRScheduler):
    def __init__(self, optimizer, warmup_periods, first_stage_epochs, second_stage_epochs, steps_per_period,
    ...
    def step(self, global_step):
        ...
        if self.global_step < self.warmup_steps:
            ...
            lr = self.learn_rate_end + 0.5 * (self.learn_rate_init - self.learn_rate_end) * (1 + np.cos(
                (self.global_step - self.warmup_steps) / (self.train_steps - self.warmup_steps) * np.pi))
    ...
    def train(self):
        ...
        for epoch in range(1, self.first_stage_epochs + self.second_stage_epochs + 1):
            ...
            with torch.no_grad():
                ...
                print(f"Evaluation finished with average loss: {test_epoch_loss:.2f} \n")
